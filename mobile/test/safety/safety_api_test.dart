...
void main() {
  ...
  group('SafetyApi.generateDatePlanShareLink', () {
    test('sends a POST and returns the full share link', () async {
      final api = SafetyApi(
        accessToken: 'a-jwt',
        client: MockClient((request) async {
          expect(request.method, 'POST');
          expect(request.url.path, '/safety/check-ins/check-in-1/share-link');
          return http.Response(
            '{"shareToken":"abc123"}',
            201,
            headers: {'content-type': 'application/json'},
      ...
      final link = await api.generateDatePlanShareLink('check-in-1');

      expect(link, endsWith('/safety/shared/abc123'));
    });

    test('throws SafetyApiException on a non-201 response', () async {
      final api = SafetyApi(
        accessToken: 'a-jwt',
        client: MockClient((request) async => http.Response('{"message":"nope"}', 404)),
      );

      expect(
        () => api.generateDatePlanShareLink('check-in-1'),
        throwsA(isA<SafetyApiException>()),
      );
    });
  });
