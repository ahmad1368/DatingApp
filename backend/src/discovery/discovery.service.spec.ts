...
describe('DiscoveryService', () => {
  ...
  describe('getDeck', () => {
    ...
    it('surfaces a shared school when the viewer and candidate match, case/whitespace-insensitively', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
      ...
      expect(deck[0].sharedSchool).toBe('nyu');
    });

    it('hides shared school when the viewer and candidate attended different schools', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
      ...
      expect(deck[0].sharedSchool).toBeNull();
    });

    it('hides shared school when either side has not set one', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
    ...
    it('shows relationship structure only when the candidate opts in', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
      ...
      expect(deck[0].relationshipStructure).toBe('Solo Polyamorous');
    });

    it('hides relationship structure when the candidate has opted out', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
      ...
      expect(deck[0].relationshipStructure).toBeNull();
    });

    it('shows kink tag badges only when the candidate opts in', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
      ...
      expect(deck[0].kinkTagBadges).toEqual(['Dominant']);
    });

    it('hides kink tag badges when the candidate has opted out', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
    ...
    it('shows a Traveler badge when the candidate is browsing via passport', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
      ...
      expect(deck[0].isTraveling).toBe(true);
    });

    it('does not show a Traveler badge for a candidate browsing normally', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
      ...
      expect(deck[0].isTraveling).toBe(false);
    });
    ...
    it('applies a min/max height range filter to the candidate query', async () => {
      ...
      expect(prisma.user.findMany).toHaveBeenCalledWith({
        where: {
          ...
          heightCm: { gte: 160, lte: 190 },
        },
        take: 60,
      });
    });

    it('applies only a min height when no max is set', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
    ...
    it('applies the religious practice level filter to the candidate query', async () => {
      prisma.user.findUnique.mockResolvedValue({
        id: USER_ID,
        latitude: null,
        longitude: null,
        ...
        activeMode: 'DATING',
        ...noFilters,
        filterReligiousPracticeLevels: ['Very Practicing', 'Devout'],
      });
      prisma.swipe.findMany.mockResolvedValueOnce([]).mockResolvedValueOnce([]);
      prisma.user.findMany.mockResolvedValue([]);
      ...
      expect(prisma.user.findMany).toHaveBeenCalledWith({
        where: {
          ...
          religiousPracticeLevel: { in: ['Very Practicing', 'Devout'] },
        },
        take: 60,
      });
    });
